...
| 桌面与 Hub 的跨设备协作 | Server 已有互联骨架，桌面接入尚未完成 |
| 多模态能力路由 | 当前覆盖文本和语音识别，其它模态仍在规划 |
...
- [x] 本地任务持久化、置顶、归档和恢复
- [x] 项目管理与按任务隔离的 Git worktree 工作区
...
- [ ] 明确 `server/` 作为 Tauri sidecar 本地运行，或由桌面端连接远程 Hub 的部署边界
- [ ] 让桌面端接入 `server/internal/interconnect/`，支持设备注册、发现与消息路由
- [ ] 向 Codex、Claude Code、opencode 等开发工具暴露用户可控的本地兼容端点
- [ ] 建立跨端协议与配置的单一事实源，支持可迁移的 Agent / Skill / MCP 配置
...
1. `server/` 是作为 Tauri sidecar 内置本地运行，还是保持独立并由桌面连接远程实例？
