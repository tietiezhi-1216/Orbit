...
| 프로젝트와 작업 공간 | 하나의 작업이 컨텍스트를 공유하며 Work는 조사와 결과물, Code는 터미널과 테스트에 최적화되고 격리 공간 사이에서 파일을 명시적으로 전달 가능 |
| 컨텍스트 관리 | 256K 창을 사용하고 약 80%에서 고정 요약을 자동 저장하며 `/compact`로 미리 압축하고 `/context`로 사용량 확인 가능 |
| 음성 받아쓰기 | 전역 단축키, 음성 인식, 모델 기반 문장 다듬기, 현재 앱에 텍스트 삽입 |
| 비주얼 제작 | Create는 이미지·동영상 생성에 집중하며 참조 이미지, 진행 상태, 재시도, 로컬 작품 관리를 지원 |
