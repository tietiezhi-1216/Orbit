...
| プロジェクトとワークスペース | 1 つのタスクでコンテキストを共有し、Work は調査と成果物、Code はターミナルとテストに最適化。分離された領域間では明示的にファイルを受け渡し可能 |
| コンテキスト管理 | 256K ウィンドウを使用し、約 80% で固定サマリーを自動保存。`/compact` で早めに圧縮し、`/context` で使用量を確認可能 |
| 音声入力 | グローバルショートカット、音声認識、モデルによる文章整形、使用中アプリへのテキスト挿入 |
| ビジュアル制作 | Create は画像・動画生成に集中し、参照画像、進捗表示、再試行、ローカル作品管理に対応 |
