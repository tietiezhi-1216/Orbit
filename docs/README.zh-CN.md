...
从 [**Releases**](https://github.com/tietiezhi-1216/Orbit/releases/latest) 获取最新构建。

- **仅支持 Apple Silicon**（arm64）· **macOS 14+** —— 不支持 Intel Mac。
- 当前为 **ad-hoc 签名**（尚未公证），首次打开会被 macOS 拦截：**右键 `Orbit.app` → 打开**，再确认。只需一次。
- 打开 `.dmg`，把 **Orbit** 拖进「应用程序」，首次使用时按提示授予 **麦克风** 与 **辅助功能** 权限。
