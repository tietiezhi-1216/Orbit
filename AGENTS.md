...
Tietiezhi is a native macOS SwiftUI app packaged with Swift Package Manager. The executable target is under `Sources/Tietiezhi/`:
...
- `./build.sh run`: build debug, assemble `Tietiezhi.app`, sign it, restart, and launch. Use for app-level verification.
...
Do not commit credentials or generated local config. Runtime settings live at `~/Library/Application Support/com.tietiezhi.app/config.json`, and API keys are currently stored as plain JSON. Verify changes to signing, bundle IDs, microphone access, Accessibility access, or clipboard/paste behavior in the real app.
